...
for i = 1:length(T_in.SolutionIndex)
    for k = 1:length(pn.prod_id)
        T_in_idx = ismember(T_in.Properties.VariableNames, [pn.prod_id{k},'_objective_']);
        T_calc_idx = ismember(T_calc.Properties.VariableNames, [pn.prod_id{k},'_objective_calc']);
